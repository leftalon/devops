...
from hlcmdb.models import Server_all
from hlcmdb.models import Game_name_all
from hlcmdb.models import Server_platform
from hlcmdb.models import IDC_name_all,Server_type_all
...
from hlcmdb.server_status import server_status_list
from django.core.paginator import Paginator
from hlcmdb.variable_db import variable_db_dict
from hlcmdb.server_search import server_search
from hlcmdb.server_search import fan_search
from django.contrib.auth.models import User,Group,Permission
from hlcmdb.permission_auth import permission_auth
from django.db.models import Q
from hlcmdb.server_search import server_search_dict
from math import ceil
from hlcmdb.query_server_message import query_server_message
from hlcmdb.pagination import pagination
from hlcmdb.pagination import pagination_django
from hlcmdb.operation_models import operation_models
from hlcmdb.models import Server_template
from hlcmdb.models import Game_type_name
from hlcmdb.server_template_dict import server_template_dict
...
def server_add(request):
	...
	elif not Game_name_all.objects.filter(game_string_id=server_id.split(".")[1]):
	    message=u"服务器所属产品%s不在游戏列表中" %(server_id.split(".")[1])
        elif not start_date_valid:
            message = u"服务器开始时间无效"
        else:
            if Server_all.objects.filter(server_id=server_id):
                message = u'服务器唯一id(%s)已经存在' %server_id
            else:
                ...
                server_t = Game_name_all.objects.get(game_string_id=server_id.split(".")[1])
                server_product = server_t.game_name
                add_sql = Server_all(server_id=server_id,server_in_ip=server_in_ip,server_out_ip=server_out_ip,server_type=server_type,server_template_id=server_template_id,server_idc=server_idc,server_company=server_company,server_product=server_product,server_status=server_status,server_start_date=server_start_date)
                if add_sql:
                    try:
                ...
                if len(one_server) == 9:
                    ...
                    if one_server[2]:
                        if not re.match(regex,one_server[2]):
                            ...
                            message=u"提供的外网ip地址非法"
                    if one_server[8]:
                        if len(one_server[8]) != 10:
                            start_date_valid = False
                        else:
                            start_date = one_server[8].split('-')
                            if len(start_date) != 3 or len(start_date[0]) != 4 or len(start_date[1]) != 2 or len(start_date[2]) != 2:
                                start_date_valid = False
                            else:
                                server_start_date = one_server[8]
                    else:
                        server_start_date = time.strftime('%Y-%m-%d',time.localtime(time.time()))
                    if Server_all.objects.filter(server_id=one_server[0]):
                        mess.append(u'%s服务器唯一标示已经存在' %one_server[0])
                        ...
                        error_many_list.append(i)
                    elif not Server_platform.objects.filter(platform_name=one_server[6]):
                        mess.append(u'%s平台不在平台列表中，请先添加平台' %(one_server[6]))
                        error_many_list.append(i)
                    elif one_server[7].title() not in server_status_list:
                        mess.append(u'服务器状态不能为%s'%(one_server[7]))
                        error_many_list.append(i)
                    elif not Game_name_all.objects.filter(game_string_id=one_server[0].split(".")[1]):
                        mess.append(u'服务器所属产品%s不在游戏列表中'%(one_server[0].split(".")[1]))
                        error_many_list.append(i)
                    elif not start_date_valid:
                        mess.append(u'时间%s为无效格式时间,格式为2013-01-01' %(one_server[8]))
                        error_many_list.append(i)
                    else:
                        #根据模板得到该模板的磁盘、cpu等信息
                        server_t = Game_name_all.objects.get(game_string_id=one_server[0].split(".")[1])
                        server_product = server_t.game_name
                        template_message = Server_template.objects.get(template_name=one_server[4])
                        server_template_id = template_message.id
                        input_db = Server_all(server_id=one_server[0],server_in_ip=one_server[1],server_out_ip=one_server[2],server_type=one_server[3],server_template_id=server_template_id,server_idc=one_server[5],server_company=one_server[6],server_product=server_product,server_status=one_server[7].title(),server_start_date=server_start_date)
                        if input_db:
                            try:
        ...
        if request.method == "POST":
            ...
            if server_end_date:
                ...
                if len(end_date) != 3 or len(end_date[0]) != 4 or len(end_date[1]) != 2 or len(end_date[2]) != 2:
                    end_date_valid = False
            off_s_id = str(id)+"."+server_id.split(".")[-1]
            if server_id == None or server_id == '':
                message = u"服务器唯一id信息不能为空"
            elif len(server_id.split('.')) != 4:
                message = u"服务器唯一id信息无效"
                ...
                server_hd = Server_template.objects.get(template_name=server_template)
                server_template_id = server_hd.id
                if Game_name_all.objects.filter(game_string_id=server_id.split(".")[1]):
                    server_t = Game_name_all.objects.get(game_string_id=server_id.split(".")[1])
                    server_product = server_t.game_name
                
                    if s_id != server_id:
                        if Server_all.objects.filter(server_id=server_id):
                            message = u'%s服务器唯一id已经存在' %server_id
                        else:
                            if Server_all.objects.filter(id__exact=id).update(server_id=server_id,server_in_ip=server_in_ip,server_out_ip=server_out_ip,server_type=server_type,server_template_id=server_template_id,server_idc=server_idc,server_company=server_company,server_product=server_product,server_status=server_status,server_start_date=server_start_date,server_end_date=server_end_date):
                    ...
                    message = u"产品线不存在"
    return render_to_response("cmdb/server_edit.html",{'server_template_dict':server_template_dict,'companyall':companyall,'templateall':templateall,'typeall':typeall,'message':message,'s_edit':s_edit,'idcall':idcall,'gameall':gameall,'server_line_status':server_status_list},context_instance=RequestContext(request))
...
def idc_all(request):
    if request.method == "GET":
        ...
        all_idc_count = idc_all_name.count()
    return render_to_response('cmdb/idc_all.html',{'idc_all_name':idc_all_name,'all_idc_count':all_idc_count},context_instance=RequestContext(request))
...
def idc_add(request):
    ...
    if request.method == "POST":
        ...
        if idc_name == None or idc_name == '':
            message = u"请输入idc名称"
        elif IDC_name_all.objects.filter(idc_name=idc_name):
	    message = u'idc名称%s已经存在' %idc_name
	else:
            idc = IDC_name_all(idc_name=idc_name,idc_address=idc_address,idc_contact=idc_contact,idc_phone=idc_phone)
            if idc:
                try:
                    idc.save()
                    ...
                    message = e
    return render_to_response('cmdb/idc_add.html',{'idc_name':idc_name,'idc_address':idc_address,'idc_contact':idc_contact,'idc_phone':idc_phone,'message':message},context_instance=RequestContext(request))
    ...
    if request.method == "POST":
        ...
        if idc_name == '' or idc_name == None:
            ...
            if idc_name != old_idc_name:
                if IDC_name_all.objects.filter(idc_name=idc_name):
                    message = u"IDC已经存在"
                else:
                    edit_idc = IDC_name_all.objects.filter(id__exact=id).update(idc_name=idc_name,idc_address=idc_address,idc_contact=idc_contact,idc_phone=idc_phone)
                    if edit_idc:
...
def game_all(request):
    ...
    return render_to_response('cmdb/game_all.html',{'game_type_name':game_type_name,'page_num':page_num,'last_page_value':last_page_value,'game_all_name':page_all_data,'page_list':page_start_end,'page_up':page_up,'page_next':page_next,'first_page':first_page,'last_page':last_page},context_instance=RequestContext(request)) 
            ...
            if len(special_list) != 0:
                ...
                if game_add:
                    try:
                        ...
                        message = e
    return render_to_response('cmdb/game_add.html',{'game_line':game_line,'oper_model':oper_models,'game_type_name':game_type_name,'oper_models':operation_models,'message':message,'game_line_status':server_status_list,'game_name':game_name,'game_appid':game_appid,'user_name':user_name,'user_phone':user_phone,'yunying_name':yunying_name,'yunying_phone':yunying_phone,'developer_name':developer_name,'developer_phone':developer_phone},context_instance=RequestContext(request))
